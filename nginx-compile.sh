...
case $OPTION in
	1)
		...
		while [[ $PCRE != "y" && $PCRE != "n" ]]; do
			read -p "       PCRE-$PCRE_VER [y/n]: " -e PCRE
		done
		while [[ $ZLIB != "y" && $ZLIB != "n" ]]; do
			read -p "       ZLIB-$ZLIB_VER [y/n]: " -e ZLIB
		done
		...
		echo ""
		read -n1 -r -p "Looks like we're ready, press any key to continue..."
		echo ""
		# Cleanup-------------------------------------------------------------------
		...
		apt-get update >> /tmp/nginx-compile.log 2>&1
		apt-get install -y autoconf unzip automake autotools-dev build-essential ca-certificates checkinstall curl debhelper dh-systemd gcc git htop libbz2-dev libexpat-dev libgd2-noxpm-dev libgd2-xpm-dev libgoogle-perftools-dev libluajit-5.1-dev libmhash-dev libpam0g-dev libpcre3 libpcre3-dev libperl-dev libssl-dev libtool libxslt1-dev make nano openssl po-debconf software-properties-common sudo tar uuid-dev wget zlib1g zlib1g-dbg zlib1g-dev
		if [ $? -eq 0 ]; then
		...
		# Then we install \o/
		echo -ne "       Installing & Buliding Deb               [..]\r"
	...
	2) # Uninstall Nginx
		...
		if [[ "$CONF" = 'y' ]]; then
			...
			rm -r /etc/nginx/ >> /tmp/nginx-compile.log 2>&1
	...
	;;
	3) # Update the script
		wget https://raw.githubusercontent.com/swoopsta/LEMP-Stack-AA/master/nginx-compile.sh -O compile.sh >> /tmp/nginx-compile.log 2>&1
